...
export const unAssignedcolumnDef = (hook: TuseModulesHook) => [
  columnHelper.display({
    cell: (props) => {
      return (
        <Box
          onClick={() => {
            hook.setViewData(props.row.original);
            hook.viewDisclosure.onOpen();
          }}
          cursor={'pointer'}
        >{`${props.row.original?.candidate?.firstName || 'No Name'}
