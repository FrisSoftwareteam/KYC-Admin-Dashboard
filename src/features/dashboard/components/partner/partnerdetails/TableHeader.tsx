...
export default function TableHeader({
  ...
  const addAgentDisclosure = useDisclosure();
  ...
  return (
    <Flex
      ...
      pr={'2.81rem'}
    >
      <Text fontWeight={500} fontSize={'.8rem'} textTransform={'capitalize'}>
        {total} {subText}
      </Text>
      ...
      <HStack gap={'.9rem'}>
        ...
        {subText === 'agents' && (
          <Button
            onClick={addAgentDisclosure.onOpen}
            fontWeight={500}
            fontFamily={'heading'}
            minH={'2rem'}
            fontSize={'.8rem'}
      ...
      <CustomModal
        ...
        isOpen={addAgentDisclosure.isOpen}
        onClose={addAgentDisclosure.onClose}
        headertext={'Add agent'}
      >
        <AddAgentModal onClose={addAgentDisclosure.onClose} />
      </CustomModal>
    </Flex>
  );
}
